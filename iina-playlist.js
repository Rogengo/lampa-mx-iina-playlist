/**
 * IINA Playlist Plugin для Lampa (v12)
 * - Кнопка [m3u Download] в ряду .torrent-filter
 * - Собирает ссылки со всех серий
 * - Название: .full-start-new__title
 * - Скачивание + фолбэки iinaSaveLast() / iinaShowLast()
 */
(function () {
    'use strict';
    if (window.__iina_playlist_v12__) return;
    window.__iina_playlist_v12__ = true;
    ...
    window.iinaSaveLast = function () {
        ...
        if (!w) {
            console.log('Popup заблокирован. Скопируй ссылку вручную:');
            console.log(d.url);
        }
    };
    ...
    window.iinaDebug = function () {
        var info = {
            'torrent-filter': $('.torrent-filter').length,
            'iina-btn': $('.torrent-filter').find('.iina-btn').length,
            'online__body': $('.online__body').length,
            'Название': getMovieTitle(),
    ...
    function init() {
        log('Плагин v12 инициализирован');
        log('Фолбэки: iinaSaveLast() — открыть M3U, iinaShowLast() — показать текст');
